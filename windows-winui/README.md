...
`%LOCALAPPDATA%\ytdl-win\state\profiles.json` before the window opens. It
carries the app's own defaults — every option unset — so selecting it is "put
the form back", not a preset that decides anything. That is deliberate: quality,
...
| Notifications | while the window is in the background, one toast when the queue finishes and one as soon as a run fails. Off switch under **When you are away**. On the pinned Windows App SDK (2.4.0), a self-contained unpackaged app cannot register for toast clicks (microsoft/WindowsAppSDK#6774), so the toast shows but clicking it does nothing yet |
| Subscriptions | the pipeline's stored sources and its hourly check, read from `ytdl --subscriptions --json` each time the pane is shown. **The app runs no timer:** the check is a Task Scheduler task the pipeline installs (`ytdl --schedule install`), so it runs whether or not the app is open (while you are signed in); the switch turns it on and off. Rows show how often, when last checked and what that found, and when next due; **Check now** queues `ytdl --run-subscriptions ID` on the Downloads queue; the menu pauses, re-times or unsubscribes. Subscribe from the Downloads form's **Subscribe…** button. Needs `0004-orchid-ochre` or later |
