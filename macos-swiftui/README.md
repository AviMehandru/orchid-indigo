...
| Notifications | while the app is in the background, one notification when the queue finishes and one as soon as a run fails; clicking brings back Downloads. Off switch under **When you are away**. macOS asks for permission the first time there is something to say. Needs a real `.app` bundle — a bare binary has no bundle identifier, and posts nothing |
| Subscriptions | the pipeline's stored sources and its hourly check, read from `ytdl --subscriptions --json` each time the pane is shown. **The app runs no timer:** the check is a launchd agent the pipeline installs (`ytdl --schedule install`), so it runs whether or not the app is open; the switch turns it on and off. Rows show how often, when last checked and what that found, and when next due; **Check now** queues `ytdl --run-subscriptions ID` on the Downloads queue; the menu pauses, re-times or unsubscribes. Subscribe from the Downloads form's **Subscribe…** button. Needs `0004-orchid-ochre` or later |
